...
- Automatic cash movements generated by cash sales and refunds (`SALE` and `REFUND`).
...
### Automatic sale and refund movements

Cash movements are integrated with the sales lifecycle.

When a sale is created with `CASH` as its payment method:

- The authenticated user must have an open cash register session.
- The backend creates a `SALE` cash movement in that session.
- The movement amount is equal to the sale total.
- The movement stores the related `sale_id`.
- The session is locked while the sale and movement are processed.

When a completed cash sale is cancelled:

- The authenticated user must have an open cash register session.
- The backend creates a `REFUND` cash movement in the current open session of the user performing the cancellation.
- The movement amount is equal to the cancelled sale total.
- The movement stores the related `sale_id`.
- The session is locked while the cancellation and refund are processed.

Sales paid with `CARD` or `TRANSFER` do not create cash movements when they are created or cancelled.

Sale creation or cancellation, inventory changes, and the corresponding automatic cash movement are executed inside the same database transaction. If any operation fails, all changes are rolled back.

`SALE` and `REFUND` cannot be submitted to `POST /api/cash-movements`.
...
```json
{
  "data": [
    ...
    },
    {
      ...
      "type": "REFUND",
      "amount": "100.00",
      "reason": "Refund for cancelled sale #15",
      "sale_id": "15",
...
Movements are returned in chronological order. `sale_id` is `null` for manual movements and contains the related sale ID for automatic `SALE` and `REFUND` movements.
